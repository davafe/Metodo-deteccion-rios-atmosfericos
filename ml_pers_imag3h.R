...
if (topo_dims_used==1){
  persim<-persim[, 1:137] #solo se usan imágenes de persistencia de dimensión 1
}
if (topo_dims_used==0){
  persim<-persim[, -(3:127)] #solo se usan imágenes de persistencia de dimensión 0
}
...
# eliminamos las 3 primeras columnas (índices y etiquetado de AR)
...
# Versión anterior del código sin usar las funciones:
# accuracy<-vector(mode="double",length=5);precision<-vector(mode="double",length=5);sensitividad<-vector(mode="double",length=5);k_fold<-vector(mode="integer",length=5);for(i in 1:5){conjunto_validacion<-dataset_equilibrado[fold[[i]],];x_val<-x[fold[[i]],];if(i!=5){conjunto_test<-dataset_equilibrado[fold[[i+1]],];x_test<-x[fold[[i+1]],];if(i==4){conjunto_entreno<-dataset_equilibrado[c(fold[[1]],fold[[2]],fold[[3]]),];x_entreno<-x[c(fold[[1]],fold[[2]],fold[[3]]),];cat(i,i+1,1,2,3,"\n")};if(i==3){conjunto_entreno<-dataset_equilibrado[c(fold[[5]],fold[[1]],fold[[2]]),];x_entreno<-x[c(fold[[5]],fold[[1]],fold[[2]]),];cat(i,i+1,5,1,2,"\n")};if(i==2){conjunto_entreno<-dataset_equilibrado[c(fold[[4]],fold[[5]],fold[[1]]),];x_entreno<-x[c(fold[[4]],fold[[5]],fold[[1]]),];cat(i,i+1,4,5,1,"\n")};if(i==1){conjunto_entreno<-dataset_equilibrado[c(fold[[3]],fold[[4]],fold[[5]]),];x_entreno<-x[c(fold[[3]],fold[[4]],fold[[5]]),];cat(i,i+1,3,4,5,"\n")}};if(i==5){conjunto_test<-dataset_equilibrado[fold[[1]],];x_test<-x[fold[[1]],];conjunto_entreno<-dataset_equilibrado[c(fold[[2]],fold[[3]],fold[[4]]),];x_entreno<-x[c(fold[[2]],fold[[3]],fold[[4]]),];cat(i,1,"\n")};accuracy_validacion_1<-vector(mode="double",length=75);y_validacion<-conjunto_validacion[,AR];y_entreno<-conjunto_entreno[,AR];y_test<-conjunto_test[,AR];if(escalar_KNN==TRUE){scaling_params<-preProcess(x_entreno,method=c("center","scale"));x_train<-predict(scaling_params,x_entreno);x_valid<-predict(scaling_params,x_val)}else{x_train<-x_entreno;x_valid<-x_val};for(k in c(1:10,seq(15,75,5))){modeloi<-knn(train=x_train,test=x_valid,cl=y_entreno,k=k,use.all=FALSE);accuracy_validacion_1[k]<-mean(modeloi==y_validacion)};cat(accuracy_validacion_1," \n");k=which.max(accuracy_validacion_1);cat(k,accuracy_validacion_1[k],"\n");k_fold[i]<-k;conjunto_entreno_and_val<-rbind(conjunto_entreno,conjunto_validacion);if(escalar_KNN==TRUE){scaling_params<-preProcess(conjunto_entreno_and_val[,-c(1,2,3,4,5,6,7)],method=c("center","scale"));x_train_and_val<-predict(scaling_params,conjunto_entreno_and_val[,-c(1,2,3,4,5,6,7)]);x_test<-predict(scaling_params,conjunto_test[,-c(1,2,3,4,5,6,7)])}else{x_train_and_val<-conjunto_entreno_and_val[,-c(1,2,3)];x_test<-conjunto_test[,-c(1,2,3)]};y_train_and_val<-c(y_entreno,y_validacion);pred_test<-knn(train=x_train_and_val,test=x_test,cl=y_train_and_val,k=k,use.all=FALSE,prob=FALSE);matriz_confusion<-table(y_test,pred_test);print(matriz_confusion);accuracy[i]=(matriz_confusion[1,1]+matriz_confusion[2,2])/sum(matriz_confusion);sensitividad[i]=matriz_confusion[2,2]/sum(matriz_confusion[2,]);precision[i]=matriz_confusion[2,2]/sum(matriz_confusion[,2])};cat(k_fold[[1]],k_fold[[2]],k_fold[[3]],k_fold[[4]],k_fold[[5]],"\n");cat(accuracy,mean(accuracy),"\n");cat(sensitividad,mean(sensitividad),"\n");cat(precision,mean(precision),"\n")
...
for (i in 1:5){
  ...
  for (c in seq(0.1, 1, 0.1)){ # antes c(0.25,1:10,14,18,23,28,33,39,47,55,64,73,82)
...
for (i in 1:5){
  ...
  for (c in c(1,2,4,8)){ #antes 1,2,4,8 o c(0.25,1,2,4) o c(0.25,0.5, 0.75,1) o 
    for (gama in seq(0.001, 0.02,0.003)){ #antes  seq(0.0055, 0.013,0.0015) o seq(0.001, 0.016,0.003) 
      for (intercept in seq(0, 0.6, 0.2)){ # antes seq(0.3, 0.7, 0.1)
        for (grado in 2:3){
...
for (i in 1:5){
  ...
  for (c in 2^(0:3)){ #antes entre potencias -1 y 2 o entre potencias -2 y 1
    for (gama in seq(0.001, 0.01,0.001)){ #antes seq(0.0001, 0.001,0.0001),  seq(0.0005, 0.001,0.0001) o seq(0.001, 0.01,0.001) o ;0.01 en vez de 0.003
      for (intercept in seq(-0.6, 0.6, 0.2)){ #antes  seq(-1.4, -0.2, 0.2) o  seq(-1.0, 0.2, 0.2)
...
# Versión anterior del código sin usar las funciones:
#accuracy_sigmoid<-vector(mode="double",length=5); precision_sigmoid<-vector(mode="double",length=5); sensitividad_sigmoid<-vector(mode="double",length=5); C_fold_sigmoid<-vector(mode="double",length=5); gamma_fold_sigmoid<-vector(mode="double",length=5); intercept_fold_sigmoid<-vector(mode="double",length=5); for(i in 1:5){ y_val<-dataset_equilibrado[fold[[i]],AR]; x_val<-x[fold[[i]],]; if(i!=5){ y_test<-dataset_equilibrado[fold[[i+1]],AR]; x_test<-x[fold[[i+1]],]; if(i==4){ y_entreno<-dataset_equilibrado[c(fold[[1]],fold[[2]],fold[[3]]),AR]; x_entreno<-x[c(fold[[1]],fold[[2]],fold[[3]]),]; cat(i,i+1,1,2,3,"\n") }; if(i==3){ x_entreno<-x[c(fold[[5]],fold[[1]],fold[[2]]),]; y_entreno<-dataset_equilibrado[c(fold[[5]],fold[[1]],fold[[2]]),AR]; cat(i,i+1,5,1,2,"\n") }; if(i==2){ y_entreno<-dataset_equilibrado[c(fold[[4]],fold[[5]],fold[[1]]),AR]; x_entreno<-x[c(fold[[4]],fold[[5]],fold[[1]]),]; cat(i,i+1,4,5,1,"\n") }; if(i==1){ y_entreno<-dataset_equilibrado[c(fold[[3]],fold[[4]],fold[[5]]),AR]; x_entreno<-x[c(fold[[3]],fold[[4]],fold[[5]]),]; cat(i,i+1,3,4,5,"\n") }}; if(i==5){ y_test<-dataset_equilibrado[fold[[1]],AR]; x_test<-x[fold[[1]],]; y_entreno<-dataset_equilibrado[c(fold[[2]],fold[[3]],fold[[4]]),AR]; x_entreno<-x[c(fold[[2]],fold[[3]],fold[[4]]),]; cat(i,1,"\n") }; if(escalar_SVM_SIGMO==TRUE){ scaling_params<-preProcess(x_entreno,method=c("center","scale")); x_entreno_preproc<-predict(scaling_params,x_entreno); x_val_preproc<-predict(scaling_params,x_val) } else { x_entreno_preproc<-x_entreno; x_val_preproc<-x_val }; acu<-matrix(NA,nrow=0,ncol=4); colnames(acu)<-c("c","gama","intercept","acc"); for(c in 2^(0:3)){ for(gama in seq(0.001,0.01,0.001)){ for(intercept in seq(-0.6,0.6,0.2)){ qdebien<-svm(x=x_entreno_preproc,y=y_entreno,scale=FALSE,kernel="sigmoid",type="C-classification",cost=c,gamma=gama,coef0=intercept); predictingcontent<-predict(qdebien,x_val_preproc); acu<-rbind(acu,c(c,gama,intercept,mean(predictingcontent==y_val))) }}}; cos<-acu[which.max(acu[,4]),1]; gamba<-acu[which.max(acu[,4]),2]; intercepto<-acu[which.max(acu[,4]),3]; C_fold_sigmoid[i]<-cos; intercept_fold_sigmoid[i]<-intercepto; gamma_fold_sigmoid[i]<-gamba; x_train_and_val<-rbind(x_entreno,x_val); y_train_and_val<-c(y_entreno,y_val); if(escalar_SVM_SIGMO==TRUE){ scaling_params<-preProcess(x_train_and_val,method=c("center","scale")); x_train_and_val_preproc<-predict(scaling_params,x_train_and_val); x_test_preproc<-predict(scaling_params,x_test) } else { x_train_and_val_preproc<-x_train_and_val; x_test_preproc<-x_test }; svm_definitivo<-svm(x=x_train_and_val_preproc,y=y_train_and_val,scale=FALSE,kernel="sigmoid",type="C-classification",cost=cos,gamma=gamba,coef0=intercepto); pred_test<-predict(svm_definitivo,x_test_preproc); confu_test<-table(y_test,pred_test); accuracy_sigmoid[i]=(confu_test[1,1]+confu_test[2,2])/sum(confu_test); sensitividad_sigmoid[i]=confu_test[2,2]/sum(confu_test[2,]); precision_sigmoid[i]=confu_test[2,2]/sum(confu_test[,2]) }; cat(accuracy_sigmoid,mean(accuracy_sigmoid),"\n"); cat(sensitividad_sigmoid,mean(sensitividad_sigmoid),"\n"); cat(precision_sigmoid,mean(precision_sigmoid),"\n"); cat(C_fold_sigmoid,"\n"); cat(gamma_fold_sigmoid,"\n"); cat(intercept_fold_sigmoid,"\n")
